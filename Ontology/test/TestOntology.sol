...
contract TestOntology {
	...
	function testAddingTermsOverAnExistingIRI() public {
		...
		Assert.equal("<asdf>", ontology.ontologies(ontology_IRI,3),"Expected value '<asdf>' for term number 3 with address 'IRI_1'");
		Assert.equal("<zxcv>", ontology.ontologies(ontology_IRI,4),"Expected value '<zxcv>' for term number 4 with address 'IRI_1'");
		Assert.equal("<kostia>", ontology.ontologies(ontology_IRI,5),"Expected value '<kostia>' for term number 5 with address 'IRI_1'");
		Assert.equal("<masha>", ontology.ontologies(ontology_IRI,6),"Expected value '<masha>' for term number 6 with address 'IRI_1'");
		Assert.equal("<sasha>", ontology.ontologies(ontology_IRI,7),"Expected value '<sasha>' for term number 7 with address 'IRI_1'");
		Assert.equal("<kasha>", ontology.ontologies(ontology_IRI,8),"Expected value '<kasha>' for term number 8 with address 'IRI_1'");
		Assert.equal("<car>", ontology.ontologies(ontology_IRI,9),"Expected value '<car>' for term number 9 with address 'IRI_1'");
	}
	...
	function testCheckRdfOntology() public {
		string memory ontology_IRI = "IRI_1";
		string memory rdf_terms = "<1234><poiu><mnbv><kostia><edcf>";
		string memory result = ontology.check_rdf(ontology_IRI,rdf_terms);

		Assert.equal("<1234><kostia>", result, "Two matches expected: '<1234><kostia>'");
	}
